...
Versioning: the version is derived from the tag itself. `[project]` declares
`dynamic = ["version"]` and `hatch-vcs` reads the newest reachable tag at build time, so
there is no version number in the repository and no bump commit — `bump-my-version` and
the `[tool.bumpversion]` table it read are gone. The `build` job's tag-versus-version
step notices the `dynamic` declaration and skips comparing the tag to a written number
(`uv version --short` exits 2 on a dynamic version); it checks the *built* distribution
filename against the tag instead, which is what catches a derived version that came out
wrong. That check is the reason the checkout uses `fetch-depth: 0`: without the tags in
view, `hatch-vcs` silently builds `0.1.dev1+g<sha>` rather than failing. `CHANGELOG.md` is
generated by git-cliff into the draft release notes.
